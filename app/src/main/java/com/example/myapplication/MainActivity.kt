...
import com.example.myapplication.utils.Calc
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.apply {
            ...
            val app: ApplicationInfo = getPackageManager()
                .getApplicationInfo(getPackageName(), PackageManager.GET_META_DATA)
            val bundle: Bundle = app.metaData
            val cam = bundle.getBoolean("cameraEnable")
            val fs = bundle.getBoolean("fsEnable")
            if(cam){
                camButton.visibility =View.VISIBLE
                startf()
            }else{
                camButton.visibility =View.GONE
            }
    ...
    private fun processTextRecognitionResult(result: Text) {
        ...
        val has:String = finalText.filter { it.isDigit() || x.contains(it.toString()) }
        val xx = has.split("+", "-", "/", "*")
        val ops = has.filter { x.contains(it.toString()) }
        if(xx.size > 1) {
            val texttocalc = xx[0] + ops[0] + xx[1]
            val calcres = Calc.eval(texttocalc)
